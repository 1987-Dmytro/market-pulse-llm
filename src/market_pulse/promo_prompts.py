...
ПРО ЩО коментар (about) — РІВНО ОДИН субʼєкт на КОЖЕН коментар з текстом:
  chain  — мережа і все, що їй належить: магазини, персонал, каси, наявність на полиці,
           ...
           ПАРТНЕР акції — банк, пошта, служба доставки, платіжний застосунок, організатор
           промо-гри — теж chain, з іменем партнера ЯК У ПОСТІ, коли комент про СЕРВІС
           партнера (підтримка, застосунок, картка, доставка партнера). Умови СПІЛЬНОЇ
           акції (ставка кешбеку, строк, чесність) — це мережа-господар поста, а партнер
           іде в note. Партнер НІКОЛИ не brand.
...
СИГНАЛ треду (signal), нуль або більше:
  жалоба    — скарга чи негативна оцінка: якість, сервіс, немає в наявності, обман,
              ...
              «Могутньо і незламно», «Розкішно. Пів дня без світла, зате свічки зі знижкою»)
  ...
  цена      — судження або питання про ЦІНУ: рівень, динаміка, порівняння, умови, а
              ...
              «це за 100 г ціна?»). Оцінка ціни як ПОКАЗНИКА якості (за такі гроші воно й
              має бути таким; дешеве добрим не буває) — це ТІЛЬКИ цена, БЕЗ жалоба:
              жалоба додається лише за претензією до мережі чи до факту — псування,
              обман, «дорого», «в іншій мережі дешевше»
...
2. ШУМ і службові → subject_type "post", subject «<msg_id кореня>», БЕЗ сигналів: самі
   ...
   «переказали колегам», «Світлину вже замінили», «напишіть у чат @varusua_bot»). Сюди ж —
   репліка-ВИПАД іншому коментаторові в суперечці, навіть коли в ній названо мережу без
   оцінки самої мережі (йдіть до конкурента; я тут не працюю). Але заявлена відмова
   купувати (більше туди не піду) — це chain + жалоба, а не шум. Рядок post — це
...
4. Форма субʼєкта — НАЗИВНИЙ відмінок. Якщо сутність є в пості — у написанні ПОСТА
   ...
   «атб», «рошен»). Якщо мережа НІДЕ в треді не названа — хендл каналу без «@».
   ФОРМА ТОВАРУ, три випадки:
   (а) товар Є в пості — «тип + бренд/назва» СЛОВАМИ ПОСТА, але БЕЗ ТМ, від, власної,
       лапок, обʼєму, ваги, відсотка й жирності, і без варіанта, коли товар один
       (морозиво Сніжинка, паста Горіхова, пиво світле Кварта);
   (б) товару в пості НЕМАЄ — тип товару УКРАЇНСЬКОЮ в називному (морозиво, ковбаса,
       оселедець, пиво, масло), а бренд — як написано в коментарі (кока кола, лейс);
   (в) коли в коментарі названо ТІЛЬКИ бренд, чиє імʼя і є товаром (кола, чипси, вода
       певної марки), — це brand, а не sku. Варіант (смак, обʼєм, жирність) додається
       ЛИШЕ коли в пості перелічено кілька варіантів одного товару.
...
6. Субʼєкт сигналу спрос: наявність / де / до коли / що всередині → ТОВАР (sku або
   ...
   цена + похвала. Питання про ЗНИЖКУ на товар — а де воно?, коли буде акція на це,
   чекаємо нових знижок — це спрос + цена ОБИДВА: питають не про товар, а про його ціну.
...
11. Тред ВНЕ ДОМЕНУ — пост, у якому немає ні мережі, ні товару (питання підписника,
    оголошення, збір, попередження): КОЖЕН рядок треду → post / «msg_id кореня», без
    винятку. Сигнали — за загальними правилами (цена за названу ціну, спрос за інтересом,
    жалоба чи похвала за тоном). Названі установи (садок, школа, ліцей) ідуть у note, а
    не в subject: вони не мережа і не бренд."""
...
6. «третій день не додзвонитися до підтримки банку-партнера» → chain / імʼя партнера як у
   пості: партнер акції — це chain, і ніколи не brand [§3]
7. під постом БЕЗ мережі й БЕЗ товару (оголошення про збір, питання підписника) кожен рядок
   треду → post / <msg_id кореня>; названу установу пиши в note [§3]
8. «а знижка на це коли буде?» → sku з поста / спрос + цена ОБИДВА: питають про ціну
   товару, а не про сам товар [§5]"""
