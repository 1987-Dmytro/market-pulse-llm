...
6. Субʼєкт сигналу спрос: наявність / де / до коли / що всередині → ТОВАР (sku або
   ...
   МЕРЕЖА (chain); прохання до каналу-агрегатора про акції ІНШОЇ мережі («Нових знижок
   атб ще не виклали?») → та мережа, explicit. Питання-претензія («Що з бонусами, у мене
   нуль») → жалоба, не спрос. Пари несуть ОБИДВА типи: «дорого» і «500 грн за картонку»
   → цена + жалоба; «класна ціна» → цена + похвала.
