...
- ultracode ON (operator): the code by hand; ONE read-only workflow on the diff — 5 lenses, 17 of 57 agents finished, 40 died on a model limit, so a
  finding whose two skeptics both died counts as «refuted» there: 22 of its 26 are UNVERIFIED, not cleared (the whole `scope-vs-ruling` lens). Acted on:
  the stop above (re-measured by me, never quoted), the Makefile comment, the README's two typed bars, and from the unverified bucket by hand
  `build_readme_results.py`'s missing caller. Clean: the `@`-spelling nit, and a consumer sweep — no broken caller, no moved pin.
