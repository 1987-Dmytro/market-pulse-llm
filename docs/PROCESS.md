...
## Harness fields (v2.4, 08.09 — ruling (ee); the audit: `docs/reviews/2026-09-08-tooling-audit.md`)
- **The launch line of every executor session is `claude --dangerously-skip-permissions`** — a FIELD of this section and of every
  paid runbook's §0, never an accident of the day: the auto-mode classifier never sees `pod create` / `pod delete` (s33's two
  denials), while the protection that counts stays — `permissions.deny` on team-lead files («Deny rules block in every mode,
  including bypassPermissions»), the sweep-refusing hook (exit 2 blocks in every mode), the money guard in the repo's own scripts.
  Belt to the braces for a session started without the flag: `permissions.allow` carries exactly `Bash(runpodctl pod create:*)` and
  `Bash(runpodctl pod delete:*)` — narrow rules resolve BEFORE the classifier, a broad `Bash(*)` is suspended by it (the docs,
  permission-modes / auto-mode-config). No other allow rule: one that never fires is deleted at the retro.
...
- **The paid session's irreversible command never meets a permission gate it can lose** (v2.3 06.09 after s33's two classifier
  denials, ≈ 16 min of anchor age; **v2.4 08.09 replaces v2.3's `--help` proof, which proved nothing**): the launch line
  `claude --dangerously-skip-permissions` and the two allow rules are FIELDS («Harness fields»); the paid runbook's §0 states the
  launch line and its gate greps the rules and the create prefix (exit ≠ 0). A permission prompt or denial inside a paid session is
  a stop, and the team lead's.
