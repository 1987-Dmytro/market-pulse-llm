# PHASE — `promo-pulse-1` (stage 1 whole: C2–C5 of `docs/SPEC-v2-promo-pulse.md` §8) — **v30 10.09 (ruling (nn)): the operator's RE-SPEC — this phase's remaining items (w3 · chain-fold · s2-promote) and everything after them now live in `docs/PHASE-ship-1.md` §2, the ONE live phase file; this file stays for its §8 DONE list (bytes) and its history, and is not launched any more (the standing prompt is v2, `docs/PROMPT-standing.md`).** v29 10.09 (ruling (mm)): the w3 premise MEASURED on my own runs and the remaining work written as §2's FEATURE LIST — «w3» (c3's 57 Маркетопт positions into the store as the third window through c3's OWN seal, the screen over ALL windows, a bad `--window` id refusing ≠ 0), «chain-fold» ((gg)2 shape (b): one Маркетопт), «s2-promote» (the 120 threads the paid dev loop read under the shipped law reach `results/promo_signals/` and the feed — today the feed is EMPTY and the screen says «not bought»), «e2e-gate»; the team lead's positions gold and the operator's gate beside them; NO paid steps remain (c3 CLOSED $0.2577 under (ll), the volume deleted, REMAINING $1.1602). Revision history v9–v28 (rulings (s)…(ll)): `git log -- docs/PHASE-promo-pulse-1.md` and the rulings file `docs/reviews/2026-08-30-plan-promo-pulse-1.md` — the live rule of every revision stays where it acts, in §2–§8 below.
...
**Remaining items:** MOVED to `docs/PHASE-ship-1.md` §2 by ruling 10.09 (nn) (the feature list with every check — w3 · chain-fold · s2-promote · serve-loop · front-1 · front-2 · e2e-ship · design-brief · positions-gold · linkedin-pack · gate); the v29 text is in this file's git history.

## 3. Registry revision — step 0 of the phase (operator's ruling 30.08, confirmed list)

DONE (v29 compaction; the full list of 30.08 is in this file's git history): revision r2 is sealed under C2's registration — `config/registry.yaml` sha `eff8ba5b…`, unchanged since `134eaba`. **A1 prices, 17 rows** (`@atb_market_official` the leaflet carrier, `@ATB_FANatik` + its group, `@atb_aktsiyi`, `@VARUS_channel`, `@ekomarket_shop`, `@epicentrk_sale`, `@foraINFO`, Маркетопт private `+Ejz6ubzm21IyMTQy` = `marketopt_private`, `@blyzenkoua`, `@silposilpo`, `@fozzyshopua`, `@sim23_simi`, `@rrozetka`, `@msuaaaa`, `@kop1chat`, `@znishkom`, `@xochydeshevshe`; image carriers → vision), **A2 voice 5**, **PAUSED 39** with `collect: false` — they STAY in the registry (removing them breaks `pulse.db`) and the SCREEN does not show them (v29: the product's population rule reaches the export), **B (Poltava) deferred to phase B**.
Every NEW channel enters through the adaptation protocol (profile → sealed hundred → gate BEFORE aggregates/train).
