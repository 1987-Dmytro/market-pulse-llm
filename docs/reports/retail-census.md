...
`к/д или с/д` = комментов/день для каналов, сообщений/день для чатов (у чата нет счётчика ответов — там прочерк, а не ноль). Реестровые строки идут без вердикта: их измерял стор, а не этот ценз. **`≥` — окно упёрлось в потолок 1200 сообщений: у 23 строк история обрезана, и ставка — нижняя граница, а не замер (потолок = 42.86 сообщ./день).**
