...
**The null was pressure-tested twice, on the operator's challenge.** (1) The authorised query terms are ('чат', 'спільнота', 'оголошення', 'барахолка') — all four select classifieds, so «no brand voice» risked being a fact about the QUERY. Sweeping the discussion genre across all 24 centres (48 queries): **«типове» returns 0 hits in every one of the 24**, «підслухано» returns 9 handles of which exactly one is a real town channel — `@svitlo5s` «Підслухано Кременчук.», 1 865 subs, comments OPEN at **0.107/day (~3 a month)** and **dairy 0.000**. Чутове returns 0 under this genre too. The gap was real and the conclusion did not move (Dv900). (2) Geolocation discovery: `contacts.getLocated` answers **0 chats for Полтава and Кременчук**, and structurally it returns only GEOCHATS — supergroups whose admin attached a location, which a chat merely named «Полтава чат» never is. Nothing about the account was published: the docs state `self_expires` is what makes a location public and «if the flag isn't set, no changes will be applied»; the calls omitted it (Dv901).
