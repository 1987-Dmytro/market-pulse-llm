...
| **A100 80GB PCIe** | 80 GB | **1.39** | Low | **> $0.80/h → rung 1 STOP, no endpoint** |
| RTX PRO 4500 | 32 GB | 0.72 | High | under the ceiling, **not A6000-class** |
...
* **32 GB against a config that is FROZEN LAW.** `config/qlora.yaml`'s own comment on the paged
  optimizer is "the 48 GB card must survive a long-sequence spike", and `max_seq_len` 1408 may not
  be lowered to compensate — editing it is forbidden and would invalidate the comparison rather than
  merely change it.
...
| A100 80GB, $1.39/h | needs the cap ceiling raised BEFORE any endpoint | faster card, so possibly cheaper in absolute dollars, but `s/step` is unmeasured: rungs 4 and 8 need re-registering first |
| RTX PRO 4500 32 GB, $0.72/h | inside the cap | not A6000-class; unmeasured s/step AND real OOM risk against a frozen `max_seq_len`, with no mid-arm checkpoint |
