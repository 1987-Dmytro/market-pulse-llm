...
**Auto-refreshed:** 2026-09-06 11:36:21 (every SessionStart)
**Branch:** `main`
...
9616af1 docs(progress): s29 — (x)'s prep is done except the registration, which anchors the line
fa0f37a docs(progress): s29 — (x)'s prep is done except the registration, which anchors the line
39022ac s29(iter5 prep): the card and the backstop are fields of the record, and an ERROR reply is read everywhere replies are read
26582d8 docs(team-lead): PROCESS «Money» v2 — the mechanics rulings (q)…(x) fixed
```
...
# Hot Cache — curated
**Last update:** 2026-09-06 (s30; стоп ЗАКРЫТ рулингом (y), ранбук §0a исправлен). Этап 1 `SPEC-v2`,
...
`--pack` стоял МЕЖДУ `--register` и коммитом, где `build_pack()`→`committed_registration()`
ОТКАЗЫВАЕТ (`git diff HEAD --quiet` на записи, которую `--register` только что переписал); теперь
ДВА коммита — запись первой, пак вторым. `make check` того HEAD расписан срезами с ОТКРЫТЫМ хвостом.
...
→ коммит пака → `make check` срезами на том HEAD → §0 предподовый `--note` (ОДНА строка леджера) →
...
пункт. · **`prereg_promo_holdout.json` ЗАПЕЧАТАН под старой sha раннера.** · **Срез `make check` — ЗАКРЫТЫЙ
список:** `ls tests/test_*.py` = **229**, хвост среза ОТКРЫТЫЙ (`'146,$p'`); сюита ~12 мин.
