...
**✅ STOP ЗАКРЫТ РУЛИНГОМ — линию подняли, окно не резали.** (a) якорь 2026-08-09 РАТИФИЦИРОВАН,
...
(якорь тот же, детерминизм `ab927a23` дважды, пин выборки сошёлся с `census_5c2` на 59 каналах):
...
СПИСКИ ИНГРЕДИЕНТОВ. Ценз сообщает и не решает — сузить ли ногу до строк с валютой — вопрос
prep-c3b/оператора.

**🔱 Развилка тимлида (Dv285), измеренная а не заявленная.** В `evidence.KINDS` нет вида «пост
прочитан и не дал ничего», `KIND_FIELDS` заморожен контрактом. Соседние ноги всегда пишут строку на
вход → их множество отвеченного точное. Нога постов при `[]`/нечитаемом ответе не пишет НИЧЕГО:
полный проход идемпотентен (вотермарк), прерванный перепокупает пустые посты. Контроль — нога
листовок на ТЕХ ЖЕ трёх ответах: 2 переспрошено против 0; через скрипт — 2 вызова транспорта
против 0. Форма фикса: четвёртый член `KINDS` + строка в `KIND_FIELDS` как у `leaflet_page`.
