...
**БРИФИНГ 5c2 — РУЛИНГ ОПЕРАТОРА ПО ВОПРОСУ 7.** Единственное, что осталось от sku-b: B′ закрыт,
третьего инструмента (13)(d) не разрешает, и решение о продуктовом чтении — оператора. Кандидат
лежит в [[skub2-b-prime-closed]] §6 и ничем не авторизован: глубину несут промо + печатный `-N%`
(оба 80/80 верны у тимлида, бейдж внутри 2пп на всех 80 парах), извлечённая старая цена остаётся —
она ТОЧНЕЕ бейджа как вход глубины (0.1670пп против 0.3212пп медианы), — но остаётся ПОМЕЧЕННОЙ:
...
**Dv210 ЗАКРЫТ.** `bar_one` читает голд из `bars.leaflet_brand_recall.gold.per_post` самой
прережки, склеивая с референсом ПО `item` (референс остаётся источником списка постов и счётчиков
посланных страниц и пинится через `gold.derived_from.sha256`). `gold_source` выбирает источник по
ФОРМЕ регистрации — есть `per_post` это B′, есть `sha256` это референс, оба или ни одного = ОТКАЗ.
Ещё четыре читателя того же файла молча предполагали resume-блок и были KeyError на форме, которую
пишет skub2. Шесть дефолтов продюсера переехали на НОВЫЕ skub2-пути, не поверх старых.

**Dv232 — МОЙ ПРОМАХ, НЕ ПОДЛЕЖИТ ВОССТАНОВЛЕНИЮ ДЛЯ ЭТОГО ПРОГОНА.** Контракт просил
предупреждения парсера (footnote/multipack/from) в записи прогона; их там нет. `warnings()` — метод
над `pack_count`/`discount_footnote`/`price_qualifier`, колонки дампа выведены из фразы планки 2 и
несут только третье, сырые ответы не хранятся. Dv199 называл это шагом skub2-run, и я пошёл в
платный прогон, не проведя его. Что всё-таки снялось из дампа: `price_from` — 3 позиции и ВСЕ ТРИ
на одной ТЕКСТОВОЙ строке, то есть «від X грн» на 108 листовых страницах не встретилось НИ РАЗУ.
Это отрицательный результат про семейство парсера, а не только дырка: три строки, которые
принимает префлайт, взяты из причин отказов v4, и как минимум одна на популяции страниц, похоже,
не встречается вовсе. Перед любым следующим позиционным прогоном — провести предупреждения в
запись.
