...
**Контракт C1 `retail-census` ИСПОЛНЕН, таблица НЕПОЛНАЯ и ДОСЧИТЫВАЕТСЯ** ($0): шаг 0 — том `mp-lora-c` удалён (листинги в отчёте,
...
делаются отдельным шагом, в этом контракте registry не тронут. **Стена FloodWait 85 352 с снимается 2026-08-28 10:21 UTC** (записана в `results/joins_5c1.jsonl` строкой `(census)`; до этого времени любой резолв хэндла её продлевает — скрипты уже отказываются стартовать). После неё одна команда досчитывает оставшиеся 125 строк, запись накопительная. Дальше по карте — C2 `positions-scale` (≤$1). Карта, деньги и живые рулинги — в `docs/STATUS.md`; читать ЕГО.
...
BLOCK, а не в прозе над ним. `make check` после всей работы: **2 failed, 4099 passed, 2 skipped** (второй красный — старый
долг `test_repair_phase4_ledger`); новых красных ноль.
