...
`--register-addendum` добавил `config/registry.yaml = eff8ba5b…` и `window_id = w2` (12 вставок / 0 удалений, повтор
ОТКАЗЫВАЕТ, тест обеих сторон). Строки выбираются по ЗАКРЕПЛЁННЫМ id: w1 — манифест по `selection_pin` + D-cut,
ПЕРЕСЧИТАННЫЙ и сверенный с `postcut_c3b :: kept.ids_sha256`; 20 постов D-cut — в ОБОИХ окнах.
...
четыре хука: **140 about + 84 signal, 0 отказов**, покрытие 140/140. **S9 $0-половина ГОТОВА:** `--dry-run` →
...
2 skipped · 7 errors** (35 красных → 20, passed +20): каждый красный — вилка. Счётная половина (j) ВЫПОЛНЕНА
(4 267 ≥ 4 266), держит только зелёность. Dv903 закрыт рулингом; 7 редов от `--window w2` были МОИ и починены.
**НЕ выводить заново:** $0.00030669/с, резерв $0.2843, хвост 60 с; 16 из 405 постов C2 без строки-улики
(14 `@epicentrk_sale`) — `runs[1].post_text` = [], потеряно на убийствах харнеса.
