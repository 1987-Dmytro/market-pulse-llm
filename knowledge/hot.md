...
(41 858) через сайт → Instagram → Telegram, но КОРПОРАТИВНЫЙ. **Личность — по бесплатному
`t.me/s/`:** так пойман `@ON_LINE_MO` — «служебна група», не промо, при цене 0.460 (Dv893).
...
**⛔ ЦЕНЗ БОЛЬШЕ НЕ READ-ONLY — ОДНА СТРОКА.** Аккаунт ВСТУПИЛ в приватный канал Маркетопта
(42 378, `read_only: false`, журнал `joins_5c1.jsonl`, обратимо `--leave`): цена 0.276, **молочка
0.241**, комменты закрыты. Но `media_share` **1.000** и ноль `грн`/`₴` — **цены внутри листовок**,
строку читает vision 5c2, не текст (Dv897).
...
**⛔ `MEMORY.md`**: 208 строк, доезжает 186; убрать строки — решение оператора, мерить
`context-census.py::loaded_memory`.
