...
**Last update:** 2026-08-16 (закрытие дня 15.08). **День дал восемь контрактов**: `cycle2-prep-a`,
`phase6a`, `phase6b`, `fix-a`, `fix-b`, `fix-c`, `probe-a`, `probe-b`. Три амендмента SPEC (3.20,
3.21, 3.22), командный центр собран, слой комментных сигналов прошёл две платные пробы. `make check`
2 347 → **2 562 / 2 skipped**. Потрачено за день по БИЛЛИНГУ **≈$0.388** GPU (probe-a $0.075 + probe-b $0.31316).
Подробности по контрактам — [[2026-08-15]]; решение дня — [[reader-probe-card-and-interface]].

**⛔ ГВАРД ОТКАЗЫВАЕТ НА `--step probe-b`, И ОН НЕПРАВ.** `REFUSED: probe-b's $0.35 cap is reached
($0.4493 spent)`, выход 1. По биллингу probe-b стоил **$0.31316** GPU и уложился в кап; $0.4493 —
это дельта баланса, в которую попал том 100 ГБ (~$0.0092/ч, тянет всегда). **Шаговый счётчик на
дельте баланса меряет СЧЁТ, а не шаг, и не останавливается** (Dv412, §10 отчёта probe-b). Цени шаг
по `runpodctl billing serverless|pods --start-time <якорь>`, исключая том ПО ВИДУ.
[[a-step-meter-on-a-balance-delta-never-stops]]

**⏭️ С ЧЕГО НАЧАТЬ ЗАВТРА.** (1) Решить, чинить ли Dv412 до следующего платного контракта — иначе
любой `--step` будет врать вверх и рано или поздно откажет. (2) Выбор оператора из четырёх вариантов
§9 отчёта probe-b. (3) Всё остальное в очереди — $0.
