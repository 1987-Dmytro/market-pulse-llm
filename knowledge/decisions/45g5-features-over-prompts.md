...
|---|---|---|---|---|---|
| replies to a comment | 3,147 / 11,338 | 882 | **23** | 120 / 258 | **62** |
...
- **Channel identity: 10 of 45.** A blanket "this pseudonym is the retailer → `unclear: true`"
  breaks 10 rows in 45. Real, and small enough to argue about.
- **Replies to a comment: 62 of 258.** A blanket "a reply to another commenter is not a
  consumer reaction" would flip **62 of the 258 judged-correct rows** — nearly a quarter of
  everything the sitting accepted. This is the "a direct accusation against the retailer
  outweighs a commenter addressee" exception (guideline rule 5, `@VARUS_channel:15587`) priced
  for the first time, and it is not an edge case.

**The two families are nearly nested, not additive.** All 7 identity refusals are also reply
refusals, and 233 of the 236 identity batch rows are. Together they cover **23 of 42**
refusals, and **19 refusals are in neither** — **14 of them name `intents`** (the food-poll
...
The 62-of-258 number is the reason a context line is the shape to try rather than a rule: a
line that says *this comment replies to another commenter* leaves the exception available to
the model, where a rule removes it.
...
- The 19 refusals in neither family are the residue that no feature discussed so far explains.
