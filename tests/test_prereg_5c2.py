...
def test_a_twelfth_marked_block_is_refused_rather_than_stripped(monkeypatch, tmp_path):
    ...
    The intruder was `amendment-3.19`, then `-3.20`, then `-3.21`, then `-3.23`, then `-3.24`, then
    `-3.25`, and each time that name became law it moved on. It has to: a second block under a name
    the producer already knows would test DUPLICATE detection, and what is being tested is that a
    name nobody has looked at cannot arrive quietly. Moving it is the one part of the landing
    manoeuvre the suite does NOT force — a duplicate still refuses, so the test still passes — and
    it was duly left behind when 3.21 landed. The assertion below is what makes the next omission
    red instead of silent, and it has now done its job on four consecutive landings: 3.23, 3.24,
    3.25 and 3.26 each walked into the planted name and this line reddened.
