...
def test_every_number_in_the_report_is_re_derived_from_the_file_that_owns_it():
    """`scripts/check_lora_c_vramprobe_report.py`, driven as a COMMAND — its exit code is the claim.

    The report says «61 of 61 re-derived»; this is what makes that sentence checkable rather than
    ...
    """
    got = subprocess.run(
        [sys.executable, str(REPO_ROOT / "scripts" / "check_lora_c_vramprobe_report.py")],
        cwd=REPO_ROOT,
        capture_output=True,
        text=True,
    )
    assert got.returncode == 0, got.stdout + got.stderr
    assert "0 FAILED" in got.stdout
