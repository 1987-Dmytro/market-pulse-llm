...
def test_the_shipped_record_still_agrees_with_the_shipped_registry():
    ...
    «Дозаявка №8»'s six with an open discussion group, and step 7's handle top-up added the two
    mothers channels it resolved — both with an open group — so it is 18 now."""
    import sys as _sys
    ...
    assert len(joins) == 18, [h for _, h in joins]
    assert {"@mandziak", "@matusi_ukr", "@mamo_nepsichuy"} <= {h for _, h in joins}
