...
paths:
  ...
  - "results/verdicts_45*"
  - "results/audit_45*"
  - "results/rescores_v3.json"
  - "results/baselines.json"
  - "scripts/build_*_pack.py"
  - "scripts/relabel*.py"
  - "scripts/precheck_uplabel.py"
  - "scripts/merge_sitting_returns.py"
  - "scripts/read_calibration_returns.py"
  - "scripts/measure_empty_drop.py"
  - "scripts/freeze_testsets.py"
  - "scripts/refreeze_v2.py"
  - "scripts/mine_sarcasm*.py"
  - "scripts/train_qlora.py"
  - "scripts/train_xlmr_baseline.py"
  - "scripts/run_baseline.py"
  - "src/market_pulse/prompts.py"
  - "src/market_pulse/scorer.py"
  - "src/market_pulse/records.py"
---
# Phase 3 / 4 / 4.5 artefacts — footguns, loaded only when one of them is touched

Moved out of `knowledge/hot.md` on 2026-08-08 at `/close`: every line below is still true, and
none of it is needed to boot a session that is not touching these files. The boot tax was 20.3K
against a 9.0K target and `scripts/context-census.py` names this exact remedy. Live state, money
and anything RunPod- or Telegram-side stayed in `hot.md`.

Verbatim, not paraphrased — each of these was written by the session that paid for it.
