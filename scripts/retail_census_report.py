...
def render(record: dict) -> str:
    ...
    lines += [
        ...
        f" их измерял стор, а не этот ценз: **строки {len(checked) + 1}–{len(rows)} — это"
        f" реестровые {len([r for r in rows if r.get('in_registry')])} каналов**, они идут"
        " вторым блоком (сначала `api`, потом `store`), чтобы сортировка не сравнивала два"
        " разных окна. Кто из сетей уже в сборе и сколько даёт — смотреть туда, а не в"
        " верх таблицы."
        f" **`≥` — окно упёрлось в потолок"
